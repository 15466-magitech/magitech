...
template<> class Spline<glm::quat> {
public:

	//Spline< Quat > uses piecewise linear interpolation:
	glm::quat at(float time) const {
		if (knots.empty()) return glm::quat();
	...
	glm::quat operator()(float time) const {
		return at(time);
	}
