...
for obj in bpy.data.objects:
    ...
    collection_tuple = obj.users_collection
    if len(collection_tuple) != 1:
        print(f'Problem with finding the collection containing object {obj.name}.\
             The documentation at https://docs.blender.org/api/current/bpy.types.Object.html#bpy.types.Object.users_collection\
             says that .users_collection returns "the collections this object is in", but my test code only showed one\
             collection (the smallest) for each object. If you get different behavior, you might need to fix this code.\
             For now, using obj.users_collection[0].')
    collection_name = collection_tuple[0].name
    if collection_name not in did_collection_names:
        collection_name_begin = len(strings)
        strings += bytes(collection_name, "utf8")
        collection_name_end = len(strings)
        did_collection_names[collection_name] = (collection_name_begin, collection_name_end)
    collection_name_begin, collection_name_end = did_collection_names[collection_name]
    index += struct.pack('I', collection_name_begin)
    index += struct.pack('I', collection_name_end)
    ...
    if len(obj.data.vertex_colors) == 0:
        ...
        if len(obj.data.vertex_colors) != 1:
            print(
                "WARNING: object '" + name + "' has multiple vertex color layers; only exporting '" + obj.data.vertex_colors.active.name + "'")
    ...
    if len(obj.data.uv_layers) == 0:
        ...
        if len(obj.data.uv_layers) != 1:
            print(
                "WARNING: object '" + name + "' has multiple texture coordinate layers; only exporting '" + obj.data.uv_layers.active.name + "'")
